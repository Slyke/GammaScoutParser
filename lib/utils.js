(function(m) {
  ...
  let Utils = (settings) => {
    ...
    retr.getVersion = (gsRaw) => {
      let res = null;
      gsRaw.split("\n").map((gsRawLine) => {
        let gsRawLineArr = gsRawLine.split(" ");
        if (gsRawLineArr[0] === "Version") {
          res = gsRawLineArr[1];
        }
      });
      return res;
    ...
    retr.getCalibration = (gsRaw) => {
      let res = "";
      let started = false;
      gsRaw.split("\n").map((gsRawLine) => {
        if (gsRawLine.length < 2) {
          started = false;
          return;
        }
        ...
        if (gsRawLine.includes("GAMMA-SCOUT SoftCal gueltig")) {
          started = true;
        }
      });
      return res;
    ...
    retr.getLogs = (gsRaw) => {
      ...
      gsRaw.split("\n").map((gsRawLine) => {
        ...
        if (gsRawLine.includes("GAMMA-SCOUT Protokoll")) {
          started = true;
        }
      });
      return res;
